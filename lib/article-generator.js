...
- Include at least 2 internal links using these exact URLs:
  - https://www.buysidebrief.com — the homepage / subscribe
  - https://www.buysidebrief.com/archive.html — past newsletter issues
  - https://www.buysidebrief.com/scorecard.html — performance scorecard
  - https://www.buysidebrief.com/#subscribe — direct subscribe link
...
## STRICT length rules for the META block (MUST follow):
- title: MUST be 50-60 characters. Count carefully. Not shorter, not longer.
- metaDescription: MUST be 140-155 characters. Count carefully. Not shorter, not longer.
- slug: lowercase, hyphenated, 3-8 words, no special characters.`;
...
function wrapArticleInPage(article) {
  ...
  const jsonLd = JSON.stringify({
    ...
    author: { '@type': 'Organization', name: 'Buyside Brief', url: 'https://www.buysidebrief.com' },
    publisher: { '@type': 'Organization', name: 'Buyside Brief', url: 'https://www.buysidebrief.com' },
    wordCount,
...
/**
 * Truncate text to fit within [min, max] character range, breaking at word boundaries.
 * If already within range, returns as-is. If too long, truncates at last word
 * boundary before max and appends ellipsis if needed. If too short, returns as-is
 * (can't safely expand without changing meaning).
 */
function constrainLength(text, min, max) {
  if (!text) return text;
  text = text.trim();
  if (text.length >= min && text.length <= max) return text;
  if (text.length <= min) return text; // too short — can't safely expand

  // Too long — truncate at word boundary near max
  // Leave room for potential ellipsis (not always needed)
  let truncated = text.slice(0, max);
  const lastSpace = truncated.lastIndexOf(' ');
  if (lastSpace > min - 4) {
    truncated = truncated.slice(0, lastSpace);
  }
  // If still too long after word-boundary cut, trim harder
  while (truncated.length > max) {
    const sp = truncated.lastIndexOf(' ');
    if (sp <= 0) break;
    truncated = truncated.slice(0, sp);
  }
  return truncated;
}
...
function validateMetadata(article) {
  ...
  article.title = constrainLength(article.title, 50, 60);
  if (article.title.length < 50) {
    warnings.push(`Title too short (${article.title.length} chars): "${article.title}"`);
  }

  article.metaDescription = constrainLength(article.metaDescription, 140, 155);
  if (article.metaDescription.length < 140) {
    warnings.push(`Meta description too short (${article.metaDescription.length} chars): "${article.metaDescription}"`);
  }

  if (warnings.length > 0) {
    console.warn('[SEO] Metadata warnings:', warnings.join(' | '));
  }
  return article;
}
...
module.exports = { generateArticle, wrapArticleInPage, constrainLength, validateMetadata };
