/**
 * Signal Scoring Engine
 * 
 * Scores parsed Form 4 filings to separate signal from noise.
 * Higher score = more likely to be a meaningful insider conviction signal.
 * 
 * Note: 52-week-low proximity boost (+20) is applied during enrichment
 * in context-enricher.js after price data is fetched.
 */

/**
 * Invalid/missing tickers to skip entirely.
 */
const INVALID_TICKERS = new Set(['NONE', 'N/A', 'NA', '', 'NULL', 'UNKNOWN']);

/**
 * Detect if an owner name looks like an institutional fund rather than
 * an individual insider. Fund buys are portfolio management, not conviction.
 * 
 * Examples that should match:
 *   "Saba Capital Management, L.P."
 *   "Andreessen Horowitz LSV Fund III, L.P."
 *   "JANA Partners Management, LP"
 *   "Kennedy Lewis Investment Holdings II LLC"
 *   "Abu Dhabi Investment Authority"
 *   "Manulife (International) Ltd"
 *   "RA CAPITAL MANAGEMENT, L.P."
 *   "STEEL PARTNERS HOLDINGS L.P."
 *   "Casdin Partners Master Fund, L.P."
 * 
 * Examples that should NOT match:
 *   "BIGLARI, SARDAR"
 *   "Cohen Ryan"
 *   "Todd Paul M"
 */
function isInstitutionalBuyer(ownerName) {
  if (!ownerName) return false;
  const name = ownerName.toUpperCase();

  // Entity suffixes
  if (/\b(L\.?P\.?|LLC|LTD|INC|CORP|PLC|CO\b|HOLDINGS|PARTNERS|GROUP)\b/.test(name)) return true;

  // Fund/management keywords
  if (/\b(FUND|CAPITAL|MANAGEMENT|INVESTMENT|ADVISORS|VENTURES|PARTNERS|TRUST CO|AUTHORITY|REINSURANCE|INSURANCE CO)\b/.test(name)) return true;

  // Parenthetical entities — e.g. "Manulife (International) Ltd"
  if (/\(.+\)/.test(name) && /\b(LTD|PTE|PTY|INC|SA|AG)\b/.test(name)) return true;

  return false;
}

/**
 * Check if a ticker is valid and should be scored.
 */
function isValidTicker(ticker) {
  if (!ticker) return false;
  const t = ticker.toUpperCase().trim();
  if (INVALID_TICKERS.has(t)) return false;
  // Must be 1-6 alphanumeric characters (standard ticker format)
  if (!/^[A-Z]{1,6}$/.test(t)) return false;
  return true;
}

/**
 * Score a single parsed Form 4 filing.
 * Returns the filing object with `score`, `signals` (reasons), and `tier` added.
 */
function scoreFiling(filing) {
  let score = 0;
  const signals = [];
  const warnings = [];

  // Skip amendments — they're corrections, not new signals
  if (filing.isAmendment) {
    return { ...filing, score: 0, signals: ['Amendment filing — skipped'], tier: 'skip', warnings };
  }

  // Skip invalid tickers
  if (!isValidTicker(filing.ticker)) {
    return { ...filing, score: 0, signals: ['Invalid or missing ticker — skipped'], tier: 'skip', warnings };
  }

  // Detect institutional buyers
  const institutional = isInstitutionalBuyer(filing.ownerName);

  // Aggregate transaction data
  const buys = filing.transactions.filter(t => t.isOpenMarketBuy);
  const sells = filing.transactions.filter(t => t.isOpenMarketSell);
  const optionExercises = filing.transactions.filter(t => t.isOptionExercise);
  const gifts = filing.transactions.filter(t => t.isGift);
  const awards = filing.transactions.filter(t => t.isAward);

  const totalBuyValue = buys.reduce((sum, t) => sum + t.totalValue, 0);
  const totalSellValue = sells.reduce((sum, t) => sum + t.totalValue, 0);
  const totalBuyShares = buys.reduce((sum, t) => sum + t.shares, 0);

  // ═══════════════════════════════
  //  INSTITUTIONAL BUYER HANDLING
  // ═══════════════════════════════

  // Institutional buyers get a flat, lower score — they're portfolio management, not conviction
  if (institutional && buys.length > 0) {
    score += 15;
    signals.push(`Institutional purchase (${filing.ownerName})`);

    // Still give size credit but capped lower than individual insiders
    if (totalBuyValue >= 5_000_000) {
      score += 15;
      signals.push(`Large fund purchase: $${formatValue(totalBuyValue)}`);
    } else if (totalBuyValue >= 1_000_000) {
      score += 10;
      signals.push(`Fund purchase: $${formatValue(totalBuyValue)}`);
    }

    // Institutional sells — even less interesting
    if (sells.length > 0 && buys.length === 0) {
      score -= 10;
      warnings.push('Institutional sell (portfolio rebalancing)');
    }

    // Skip the rest of the individual insider scoring
    let tier = 'skip';
    if (score >= 100) tier = 'strong_signal';
    else if (score >= 75) tier = 'top_pick';
    else if (score >= 45) tier = 'feature';
    else if (score >= 25) tier = 'mention';

    return {
      ...filing,
      score,
      signals,
      warnings,
      tier,
      isInstitutional: true,
      summary: {
        totalBuyValue,
        totalSellValue,
        totalBuyShares,
        buyCount: buys.length,
        sellCount: sells.length,
        optionExerciseCount: optionExercises.length,
        giftCount: gifts.length,
      },
    };
  }

  // ═══════════════════════════════
  //  BOOST SIGNALS (individual insiders)
  // ═══════════════════════════════

  // C-suite buyer — CEO/CFO/COO buying is the strongest individual signal
  const title = (filing.officerTitle || '').toUpperCase();
  const isCsuite = /\b(CEO|CFO|COO|CHIEF|PRESIDENT)\b/.test(title);
  const isVP = /\b(VP|VICE PRESIDENT|SVP|EVP|SENIOR VICE|EXECUTIVE VICE)\b/.test(title);
  if (isCsuite && buys.length > 0) {
    score += 30;
    signals.push(`C-suite purchase (${filing.officerTitle})`);
  }

  // VP/SVP/EVP buying — senior officers with real operational visibility
  if (isVP && !isCsuite && buys.length > 0) {
    score += 15;
    signals.push(`Senior officer purchase (${filing.officerTitle})`);
  }

  // Director buying — board-level conviction
  if (filing.isDirector && !isCsuite && !isVP && buys.length > 0) {
    score += 20;
    signals.push('Director purchase');
  }

  // 10% owner buying — only if NOT institutional (already handled above)
  if (filing.isTenPercentOwner && buys.length > 0) {
    score += 20;
    signals.push('10%+ owner purchase');
  }

  // Named officer buying (any title) — baseline officer signal
  if (filing.isOfficer && !isCsuite && !isVP && buys.length > 0) {
    score += 5;
    signals.push('Officer purchase');
  }

  // Mega purchase — >$5M (extremely rare, very high conviction)
  if (totalBuyValue >= 5_000_000) {
    score += 45;
    signals.push(`Mega purchase: $${formatValue(totalBuyValue)}`);
  }
  // Major purchase — >$1M
  else if (totalBuyValue >= 1_000_000) {
    score += 35;
    signals.push(`Major purchase: $${formatValue(totalBuyValue)}`);
  }
  // Large purchase — >$500K
  else if (totalBuyValue >= 500_000) {
    score += 25;
    signals.push(`Large purchase: $${formatValue(totalBuyValue)}`);
  }
  // Medium purchase — >$100K
  else if (totalBuyValue >= 100_000) {
    score += 15;
    signals.push(`Notable purchase: $${formatValue(totalBuyValue)}`);
  }
  // Moderate purchase — >$25K
  else if (totalBuyValue >= 25_000) {
    score += 10;
    signals.push(`Purchase: $${formatValue(totalBuyValue)}`);
  }

  // No 10b5-1 plan — discretionary trade (not pre-scheduled)
  if (!filing.has10b51Plan && buys.length > 0) {
    score += 10;
    signals.push('Discretionary (no 10b5-1 plan)');
  }

  // Open market buy exists at all — baseline signal
  if (buys.length > 0 && score === 0) {
    score += 10;
    signals.push('Open market purchase');
  }

  // ═══════════════════════════════
  //  REDUCE SIGNALS (negative)
  // ═══════════════════════════════

  // Only option exercises — usually mechanical, not conviction
  if (optionExercises.length > 0 && buys.length === 0 && sells.length === 0) {
    score -= 20;
    warnings.push('Option exercise only (likely mechanical)');
  }

  // Gift/estate transfer — not a market signal
  if (gifts.length > 0 && buys.length === 0 && sells.length === 0) {
    score -= 30;
    warnings.push('Gift/transfer (not market signal)');
  }

  // Award only — company-granted, not insider-initiated
  if (awards.length > 0 && buys.length === 0 && sells.length === 0) {
    score -= 20;
    warnings.push('Award/grant only (company-initiated)');
  }

  // Tiny purchase — <$10K
  if (totalBuyValue > 0 && totalBuyValue < 10_000) {
    score -= 15;
    warnings.push(`Small purchase ($${formatValue(totalBuyValue)})`);
  }

  // C-suite selling — weaker signal (many legit reasons)
  if (isCsuite && sells.length > 0 && buys.length === 0) {
    score -= 10;
    warnings.push(`C-suite sell (${filing.officerTitle})`);
  }

  // Pre-scheduled sell — even weaker
  if (filing.has10b51Plan && sells.length > 0) {
    score -= 15;
    warnings.push('Pre-scheduled sale (10b5-1 plan)');
  }

  // ═══════════════════════════════
  //  TIER ASSIGNMENT
  // ═══════════════════════════════

  let tier = 'skip';
  if (score >= 100) tier = 'strong_signal';
  else if (score >= 75) tier = 'top_pick';
  else if (score >= 45) tier = 'feature';
  else if (score >= 25) tier = 'mention';

  return {
    ...filing,
    score,
    signals,
    warnings,
    tier,
    isInstitutional: false,
    summary: {
      totalBuyValue,
      totalSellValue,
      totalBuyShares,
      buyCount: buys.length,
      sellCount: sells.length,
      optionExerciseCount: optionExercises.length,
      giftCount: gifts.length,
    },
  };
}

/**
 * Score an array of filings and detect cluster buying.
 * Cluster buying: 3+ insiders at the same company buying in the same batch.
 */
function scoreAllFilings(filings) {
  // First pass: score individually
  let scored = filings.map(f => scoreFiling(f));

  // Second pass: detect cluster buying (by ticker) — only count individual insiders
  const tickerGroups = {};
  for (const filing of scored) {
    if (!filing.ticker) continue;
    const key = filing.ticker.toUpperCase();
    if (!tickerGroups[key]) tickerGroups[key] = [];
    if (filing.summary && filing.summary.buyCount > 0 && !filing.isInstitutional) {
      tickerGroups[key].push(filing);
    }
  }

  // Cluster pass — weights updated 2026-05-09 from cluster buy backtest
  // (research/cluster-buy-signal/). 12 months, N=7d window, d90 excess return vs SPY:
  //   - Paired (2):     +3.46pp vs solo, p=0.21 — direction-positive, not significant
  //   - Cluster (3-4):  -3.55pp vs solo, p=0.11 — directionally negative
  //   - Mega (5+):      -6.94pp vs solo, p<0.0001 — strongly negative, robust
  // Bucketed cluster vs solo crossed the pre-registered "reduce or kill" threshold.
  for (const [ticker, group] of Object.entries(tickerGroups)) {
    if (group.length >= 5) {
      // Mega-cluster: contrary indicator. Likely capitulation/stampede dynamic.
      for (const filing of group) {
        filing.score -= 20;
        filing.signals.push(`Mega-cluster warning: ${group.length} insiders piling into $${ticker} (contrary indicator)`);
        if (filing.score >= 100) filing.tier = 'strong_signal';
        else if (filing.score >= 75) filing.tier = 'top_pick';
        else if (filing.score >= 45) filing.tier = 'feature';
        else if (filing.score >= 25) filing.tier = 'mention';
        else filing.tier = 'skip';
      }
    } else if (group.length >= 3) {
      // Cluster (3-4): null/directionally negative. Surface for editorial transparency
      // but don't move the score.
      for (const filing of group) {
        filing.signals.push(`Cluster activity: ${group.length} insiders at $${ticker}`);
      }
    } else if (group.length === 2) {
      // Paired: direction-positive, not significant. Hold pending independent retest.
      for (const filing of group) {
        filing.score += 10;
        filing.signals.push(`Paired buying: 2 insiders at $${ticker}`);
        if (filing.score >= 100) filing.tier = 'strong_signal';
        else if (filing.score >= 75) filing.tier = 'top_pick';
        else if (filing.score >= 45) filing.tier = 'feature';
        else if (filing.score >= 25) filing.tier = 'mention';
      }
    }
  }

  // Sort by score descending
  scored.sort((a, b) => b.score - a.score);

  return scored;
}

/**
 * Filter scored filings into digest-ready categories.
 */
function categorizeForDigest(scoredFilings) {
  const topPicks = scoredFilings.filter(f => f.tier === 'top_pick' || f.tier === 'strong_signal');
  const featured = scoredFilings.filter(f => f.tier === 'feature');
  const mentions = scoredFilings.filter(f => f.tier === 'mention');
  const notable_sells = scoredFilings.filter(f =>
    f.summary && f.summary.sellCount > 0 && f.summary.totalSellValue >= 100_000 && f.tier !== 'skip'
  );

  return {
    topPicks,
    featured,
    mentions,
    notable_sells,
    totalProcessed: scoredFilings.length,
    totalFeatured: topPicks.length + featured.length,
  };
}

// --- Helpers ---

function formatValue(num) {
  if (num >= 1_000_000) return `${(num / 1_000_000).toFixed(1)}M`;
  if (num >= 1_000) return `${(num / 1_000).toFixed(0)}K`;
  return num.toFixed(0);
}

module.exports = {
  scoreFiling,
  scoreAllFilings,
  categorizeForDigest,
  formatValue,
  isInstitutionalBuyer,
  isValidTicker,
  INVALID_TICKERS,
};
