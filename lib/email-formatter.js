 ...
 */
function formatDigestEmail(categorized, date, scorecardHtml) {
...
<tr><td style="padding:24px 20px;">
  <div style="background:#ffffff;border:1px solid #e0dbd3;border-radius:12px;padding:24px;">
    ...
    <h2 style="margin:0 0 10px;font-size:22px;color:#1a1a1a;font-family:Georgia,serif;font-weight:normal;">
      $${esc(pick.ticker)} — ${esc(pick.issuerName)}
    </h2>
  ...
  const rows = filings.map(f => `
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #e0dbd3;font-family:-apple-system,Helvetica,Arial,sans-serif;">
        <strong style="color:#1a1a1a;">$${esc(f.ticker)}</strong>
        <span style="color:#7a7a7a;"> — ${esc(f.officerTitle || (f.isDirector ? 'Director' : 'Insider'))}</span>
        <span style="color:#1a7a4c;font-weight:600;"> bought $${formatValue(f.summary.totalBuyValue)}</span>
        <span style="color:#b0a99f;"> &middot; Score: ${f.score}</span>
      </td>
    </tr>`).join('');
