...
<tr><td style="padding:24px 20px;">
  <div style="background:#ffffff;border:1px solid #e0dbd3;border-radius:12px;padding:24px;">
    ...
    <p style="margin:0 0 12px;font-size:13px;color:#7a7a7a;font-family:-apple-system,Helvetica,Arial,sans-serif;">
      ...
      &middot; Signal score: <strong style="color:#1a7a4c;">${pick.displayScore || pick.score}/100</strong>
    ...
    <tr>
      <td style="padding:16px 0;border-bottom:1px solid #e0dbd3;">
        <p style="margin:0 0 4px;font-family:-apple-system,Helvetica,Arial,sans-serif;">
          ...
          <span style="color:#b0a99f;font-size:12px;font-family:'JetBrains Mono',monospace;"> Score: ${f.displayScore || f.score}</span>
...
function formatMentions(filings) {
  const items = filings.slice(0, 5).map(f =>
    `&bull; ${tickerLink(f.ticker)} — ${esc(f.ownerName)} ${f.summary.buyCount > 0 ? 'bought' : 'sold'} $${formatValue(f.summary.buyCount > 0 ? f.summary.totalBuyValue : f.summary.totalSellValue)} &middot; Score: ${f.displayScore || f.score}`
  ).join('<br>');
...
module.exports = { formatDigestEmail, getTimingPhrase };
