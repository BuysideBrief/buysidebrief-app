...
function parseEftsHit(hit) {
  ...
  // _id format is "ACCESSION:filename.xml" e.g. "0001897926-26-000002:ownership.xml"
  const hitId = hit._id || '';
  ...
  // Build direct URL to the XML if we have both pieces
  let fileUrl = '';
  if (accessionNumber && cikClean && xmlFilename) {
    const accClean = accessionNumber.replace(/-/g, '');
    fileUrl = `https://www.sec.gov/Archives/edgar/data/${cikClean}/${accClean}/${xmlFilename}`;
  }
...
async function fetchAndParseForm4(filing) {
  try {
    ...
    // Strategy 1: If we have a direct URL to the XML, use it
    if (filing.fileUrl) {
      const result = await fetchXmlDirect(filing.fileUrl, filing);
      if (result) return result;
    }
    
    // Strategy 2: Fetch the filing index.json to find the XML
    const accClean = accessionNumber.replace(/-/g, '');
    ...
    // Find the ownership XML document
...
async function fetchXmlDirect(url, filing) {
  try {
    ...
    const res = await fetch(fullUrl, { headers: { 'User-Agent': USER_AGENT }, signal: AbortSignal.timeout(FETCH_TIMEOUT_MS) });
    await sleep(RATE_LIMIT_MS);
...
function extractXmlValue(xml, tag) {
  ...
  // Decode XML entities so downstream code gets clean text
  return context.trim()
    .replace(/&amp;/g, '&')
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>')
