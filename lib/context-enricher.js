...
const { getQuote, calculateReturn, get52WeekRange } = require('./price-helper');
 ...
 * Adds `whyItMatters` (string), `insiderHistory` (object), and `priceContext` (52-week data).
 */
async function enrichFiling(filing) {
  ...
  // Get current price + 52-week range if we have price API keys
  try {
    if (filing.ticker && (process.env.FINNHUB_API_KEY || process.env.ALPHA_VANTAGE_KEY)) {
      ...
      // 52-week range lookup
      const range = await get52WeekRange(filing.ticker);
      if (range) {
        enriched.priceContext = range;

        // Apply score boost for buying near 52-week low
        if (range.nearLow && filing.summary?.buyCount > 0) {
          enriched.score = (enriched.score || 0) + 20;
          enriched.signals = [...(enriched.signals || []),
            `Buying near 52-week low ($${range.low52w}, currently ${range.pctFromLow.toFixed(0)}% above)`
          ];

          // Re-evaluate tier after boost
          if (enriched.score >= 100) enriched.tier = 'strong_signal';
          else if (enriched.score >= 75) enriched.tier = 'top_pick';
          else if (enriched.score >= 45) enriched.tier = 'feature';
          else if (enriched.score >= 25) enriched.tier = 'mention';
        }

        // Note if buying near 52-week high (not a penalty, just context)
        if (range.nearHigh && filing.summary?.buyCount > 0) {
          enriched.signals = [...(enriched.signals || []),
            `Buying near 52-week high ($${range.high52w})`
          ];
        }
      }
    }
  ...
  // Generate the "Why it matters" blurb (after price context is available)
  enriched.whyItMatters = generateWhyItMatters(enriched);
...
function generateWhyItMatters(filing) {
  ...
  if (filing.tier === 'top_pick' || filing.tier === 'strong_signal') {
    // C-suite buying
  ...
  // Cluster buying (3-4 insiders) — surfaced but not a positive signal in our backtest
  const clusterSignal = filing.signals.find(s => /Cluster activity/i.test(s));
  if (clusterSignal) {
    parts.push(`Multiple insiders bought $${ticker} in the same window. Worth noting, though our backtest didn't find that 3-4 insider clusters reliably outperform solo buys.`);
  }

  // Mega-cluster warning (5+ insiders) — contrary indicator
  const megaSignal = filing.signals.find(s => /Mega-cluster warning/i.test(s));
  if (megaSignal) {
    parts.push(`Five or more insiders bought $${ticker} in the same window. Counterintuitively, our backtest found this pattern underperforms — possibly a capitulation or stampede signal rather than conviction.`);
  }

  // Paired buying (2 insiders)
  const pairedSignal = filing.signals.find(s => /Paired buying/i.test(s));
  if (pairedSignal) {
    parts.push(`Two insiders bought around the same time. Our data suggests paired buying tracks slightly better than solo buys, though the effect isn't statistically conclusive.`);
  }
  ...
  // 52-week low proximity — the conviction kicker
  if (filing.priceContext && filing.priceContext.nearLow && filing.summary.buyCount > 0) {
    const pct = filing.priceContext.pctFromLow.toFixed(0);
    const low = filing.priceContext.low52w;
    const high = filing.priceContext.high52w;
    parts.push(`The stock is trading just ${pct}% above its 52-week low of $${low} (the high was $${high}). Buying near the bottom of the range is one of the strongest conviction signals — this insider thinks the market has it wrong.`);
  } else if (filing.priceContext && filing.priceContext.nearHigh && filing.summary.buyCount > 0) {
    parts.push(`Worth noting: the stock is near its 52-week high. This insider is buying at elevated levels, which suggests they see more upside ahead.`);
  } else if (filing.priceContext && filing.summary.buyCount > 0) {
    const down = Math.abs(filing.priceContext.pctFromHigh).toFixed(0);
    if (filing.priceContext.pctFromHigh < -25) {
      parts.push(`The stock is down ${down}% from its 52-week high. Buying on a significant pullback often signals contrarian conviction.`);
    }
  }
...
async function enrichAllFilings(scoredFilings) {
  const toEnrich = scoredFilings.filter(f =>
    f.tier === 'strong_signal' || f.tier === 'top_pick' || f.tier === 'feature'
  );
