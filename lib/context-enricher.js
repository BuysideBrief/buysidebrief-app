    ...
    if (filing.ticker && (process.env.FINNHUB_API_KEY || process.env.ALPHA_VANTAGE_KEY)) {
      ...
      if (range) {
        ...
        if (range.nearLow && filing.summary?.buyCount > 0) {
          ...
          else if (enriched.score >= 50) enriched.tier = 'feature';
...
function generateWhyItMatters(filing) {
  ...
  if (clusterSignal) {
    parts.push(`Multiple insiders buying the same stock in the same window is notable — it suggests shared conviction at current prices.`);
  }

  // Paired buying
  const pairedSignal = filing.signals.find(s => /paired/i.test(s));
  if (pairedSignal) {
    parts.push(`Two insiders buying around the same time is worth noting. Worth watching to see if more follow.`);
  }
