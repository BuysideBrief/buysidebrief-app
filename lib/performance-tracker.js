...
if (redisUrl && redisToken) {
  ...
  try {
    ...
    kv = {
      ...
      smembers: async () => [],
      pipeline: () => {
        const p = { set: () => p, zadd: () => p, sadd: () => p, exec: async () => [] };
        return p;
      },
    };
  }
...
async function recordNewPicks(scoredFilings) {
  const { getBestEntryPrice, validateEntryPrice } = require('./pick-filters');
  const today = new Date().toISOString().split('T')[0];
  let recorded = 0;

  const toRecord = scoredFilings.filter(f =>
    (f.tier === 'strong_signal' || f.tier === 'top_pick' || f.tier === 'feature') && f.summary.buyCount > 0
  );
  ...
  for (const f of toRecord) {
    ...
    // Use weighted average entry price instead of just first transaction
    let entryPrice = getBestEntryPrice(f);

    // Validate entry price against current market quote
    try {
      const quote = await getQuote(f.ticker);
      if (quote && quote.price) {
        const validation = validateEntryPrice(entryPrice, quote.price, f);
        if (validation.price !== null) {
          entryPrice = validation.price;
        } else {
          // Entry price is suspicious and no good alternative found
          // Still record the pick but flag it
          console.log(`  ⚠️ Suspicious entry price for $${f.ticker}: $${entryPrice} vs current $${quote.price.toFixed(2)} — ${validation.reason || 'excluded'}`);
          entryPrice = null; // Don't record a bad price
        }
      }
    } catch (e) {
      // Price validation is optional — don't fail the pipeline
    }
    ...
    pickWrites.push({ pickId, pick });
    recorded++;

    if (f.ownerCik) {
      ceoQueue.push({ filing: f, date: today, entryPrice });
    }
  }

  // Batch all pick writes via pipeline
  if (pickWrites.length > 0) {
    const pipe = kv.pipeline();
    const now = Date.now();
    for (const { pickId, pick } of pickWrites) {
      pipe.set(pickId, pick);
      pipe.zadd('picks:index', { score: now, member: pickId });
    }
    await pipe.exec();
  }

  // CEO profile updates are read-modify-write, run sequentially
  for (const { filing, date, entryPrice } of ceoQueue) {
    await recordCeoPick(filing, date, entryPrice);
  }
...
async function updateAllReturns() {
  ...
  // Phase 1: Load all picks from Redis
  const picks = await Promise.all(pickIds.map(id => kv.get(id)));
  const validPicks = [];
  for (let i = 0; i < pickIds.length; i++) {
    const pick = picks[i];
    if (pick && pick.ticker && pick.entryPrice) {
      validPicks.push({ id: pickIds[i], pick });
    }
  }

  // Phase 2: Collect unique tickers and fetch prices in parallel chunks
  const uniqueTickers = [...new Set(validPicks.map(p => p.pick.ticker))];
  const priceMap = await chunkedPriceFetch(uniqueTickers);

  // Phase 3: Calculate returns and write updates
  let updated = 0;
  const pickWrites = [];
  const ceoUpdates = [];

  for (const { id, pick } of validPicks) {
    const quote = priceMap[pick.ticker];
    ...
    pickWrites.push({ key: id, value: pick });
    updated++;

    if (pick.ownerCik) {
      ceoUpdates.push({ ownerCik: pick.ownerCik, pickId: id, rounded, daysSince });
    }
  }

  // Batch Redis writes via pipeline if available, otherwise sequential
  if (typeof kv.pipeline === 'function' && pickWrites.length > 0) {
    const pipe = kv.pipeline();
    for (const { key, value } of pickWrites) {
      pipe.set(key, value);
    }
    await pipe.exec();
  } else {
    await Promise.all(pickWrites.map(({ key, value }) => kv.set(key, value)));
  }

  // CEO profile updates (these read-modify-write, so run sequentially)
  for (const { ownerCik, pickId, rounded, daysSince } of ceoUpdates) {
    try {
      await updateCeoPickReturn(ownerCik, pickId, rounded, daysSince);
    } catch (e) {
...
/**
 * Fetch prices for multiple tickers in parallel, chunked to respect rate limits.
 * @param {string[]} tickers - Unique ticker symbols
 * @param {number} chunkSize - Max concurrent requests per chunk (default 10)
 * @returns {Object} Map of ticker → quote (or null on failure)
 */
async function chunkedPriceFetch(tickers, chunkSize = 10) {
  const results = {};
  for (let i = 0; i < tickers.length; i += chunkSize) {
    const chunk = tickers.slice(i, i + chunkSize);
    const prices = await Promise.all(
      chunk.map(ticker => getQuote(ticker).catch(err => {
        console.error(`Price fetch failed for ${ticker}:`, err.message);
        return null;
      }))
    );
    chunk.forEach((ticker, idx) => { results[ticker] = prices[idx]; });
    if (i + chunkSize < tickers.length) {
      await new Promise(r => setTimeout(r, 1100)); // rate limit buffer
    }
  }
  return results;
}
...
async function generateScorecard() {
  ...
  // Batched read — fetches all picks in a single Redis call instead of N+1
  const results = await kv.mget(...pickIds);
  const picks = results.filter(pick =>
    pick && pick.entryPrice && pick.currentReturn !== null && pick.currentReturn !== undefined
  );
...
async function recordCeoPick(filing, date, entryPrice) {
  ...
  const pipe = kv.pipeline();
  pipe.set(ceoKey, profile);
  pipe.sadd('ceo:index', filing.ownerCik);
  await pipe.exec();
}
...
async function getAllCeoProfiles() {
  ...
  // Batched read — single mget instead of N sequential gets
  const keys = ciks.map(cik => `ceo:${cik}`);
  const results = await kv.mget(...keys);
  const profiles = results.filter(p => p && p.totalPicks > 0);
...
function formatScorecardForEmail(scorecard) {
  // Only show scorecard when we have picks with actual return data
  if (!scorecard || scorecard.totalPicks === 0) return '';
  if (scorecard.winRate === undefined || scorecard.winRate === null) return '';
  if (!scorecard.recentPicks || scorecard.recentPicks.filter(p => p.currentReturn !== null && p.currentReturn !== undefined).length === 0) return '';

  const picksWithReturns = scorecard.recentPicks.filter(p => p.currentReturn !== null && p.currentReturn !== undefined);

  const rows = picksWithReturns.slice(0, 5).map(p => {
  ...
  // Build the timeframe stats line — only include defined values
  const timeframes = [];
  if (scorecard.avg30dReturn !== null && scorecard.avg30dReturn !== undefined) timeframes.push(`30d: ${scorecard.avg30dReturn > 0 ? '+' : ''}${scorecard.avg30dReturn}%`);
  if (scorecard.avg60dReturn !== null && scorecard.avg60dReturn !== undefined) timeframes.push(`60d: ${scorecard.avg60dReturn > 0 ? '+' : ''}${scorecard.avg60dReturn}%`);
  if (scorecard.avg90dReturn !== null && scorecard.avg90dReturn !== undefined) timeframes.push(`90d: ${scorecard.avg90dReturn > 0 ? '+' : ''}${scorecard.avg90dReturn}%`);
  if (scorecard.avgYtdReturn !== null && scorecard.avgYtdReturn !== undefined) timeframes.push(`YTD: ${scorecard.avgYtdReturn > 0 ? '+' : ''}${scorecard.avgYtdReturn}%`);

  const sp500Line = (scorecard.sp500Return !== null && scorecard.sp500Return !== undefined)
    ? ` &middot; S&amp;P 500: ${scorecard.sp500Return > 0 ? '+' : ''}${scorecard.sp500Return}%`
    : '';
...
/**
 * Load recent picks from Redis for repeat insider dampening.
 * Returns picks from the last N days.
 */
async function loadRecentPicks(daysBack = 30) {
  const pickIds = await kv.zrange('picks:index', 0, -1);
  ...
  // Batched read — single mget instead of N sequential gets
  const results = await kv.mget(...pickIds);
  return results.filter(pick =>
    pick && pick.entryDate && new Date(pick.entryDate).getTime() >= cutoff
  );
}
