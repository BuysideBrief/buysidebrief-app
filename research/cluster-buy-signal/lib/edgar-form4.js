...
const { bucketRole } = require('./role-bucket');
...
function parseForm4Xml(xml, meta) {
  ...
  const role = bucketRole({ officerTitle, isOfficer, isDirector, isTenPercent });
...
/**
 * Pull all Form 4 P-transactions for a date range, checkpointing per-day.
 */
async function pullForm4Range(startDate, endDate, opts = {}) {
  ...
  for (const date of dates) {
    ...
    for (const r of results) {
      ...
      if (r && r.transactions && r.transactions.length) buys.push(r);
    }
